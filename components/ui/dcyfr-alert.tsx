  ...
  "relative w-full rounded-lg border px-4 py-3 text-sm [&>svg]:absolute [&>svg]:left-4 [&>svg]:top-4 [&>svg]:size-4 [&>svg+div]:-translate-y-0.5 [&>svg~*]:pl-7",
  {
    variants: {
      variant: {
        ...
        secure:
          "bg-secure/5 text-secure border-secure/30 [&>svg]:text-secure",
        success:
          "bg-success/5 text-success border-success/40 [&>svg]:text-success",
        warning:
          "bg-warning/5 text-warning border-warning/40 [&>svg]:text-warning",
        danger:
          "bg-destructive/5 text-destructive border-destructive/50 [&>svg]:text-destructive dark:border-destructive",
        ghostly:
          "bg-muted/40 text-foreground border-border/40 [&>svg]:text-muted-foreground",
      },
    },
    defaultVariants: {
      variant: "info",
...
export interface DcyfrAlertProps
  extends React.HTMLAttributes<HTMLDivElement>,
    VariantProps<typeof dcyfrAlertVariants> {}
